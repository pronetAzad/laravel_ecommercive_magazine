    ...
    <div class="container">
        <div class="bg-content">
            <a href="{{ route('siparisler') }}" class="btn btn-xs btn-primary">
                <i class="glyphicon glyphicon-arrow-left"></i> Siparislere Don
            </a>
            ...
            <table class="table table-bordererd table-hover">
                ...
                @foreach($siparis->sepet->sepet_urunler as $sepet_urun)
                    <tr>
                        ...
                        <td>{{ $sepet_urun->fiyati }} Azn</td>
                        <td>{{ $sepet_urun->adet }}</td>
                        <td>{{ $sepet_urun->fiyati * $sepet_urun->adet }} Azn</td>
                        <td>{{ $sepet_urun->durum }}</td>
                    </tr>
                @endforeach
                <tr>
                    <th colspan="4" class="text-right">Toplam Tutar</th>
                    <td colspan="2">{{ $siparis->siparis_tutari }} Azn</td>
                </tr>
                <tr>
                    <th colspan="4" class="text-right">Toplam Tutar (KDV'li)</th>
                    <td colspan="2">{{ $siparis->siparis_tutari * ((100+config('cart.tax'))/100) }} Azn</td>
                </tr>
                <tr>
                    <th colspan="4" class="text-right">Siparis durum</th>
                    <td colspan="2">{{ $siparis->durum }} Azn</td>
                </tr>
            </table>
        </div>
    </div>
