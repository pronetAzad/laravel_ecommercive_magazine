    ...
    public function index()
    {
        if(!auth()->check())
        {
            return redirect()->route('kullanici.oturumac')
                ->with('mesaj_tur', 'info')
                ->with('mesaj', 'Ödeme işleri için oturum açmaniz ve ya kullanici kaydi yapmaniz gerekmektedir');
        }
        else if(count(Cart::content()) == 0)
        {
            return redirect()->route('anasayfa')
                ->with('mesaj_tur', 'info')
                ->with('mesaj', 'Odeme isleri icin sepetinizde bir urun bulunmalidir.');
        }
